...
pub fn generate(types: &[TypeInfo], cfg: &JavaConfig, asn_defs: &HashMap<String, String>) {
    fs::create_dir_all(&cfg.out_dir).expect("failed to create output directory");
    for t in types {
        let code = gen_class(t, types, &cfg.prefix, &cfg.default_enc, &cfg.package, asn_defs);
        fs::write(
    ...
    .unwrap();
    println!(
        "✓ generated {} Java classes (incl. {}Native.java) in {:?}",
        types.len(),
        cfg.prefix,
        cfg.out_dir
    );
...
fn gen_class(
    ...
    // Include ASN.1 definition as a doc comment
