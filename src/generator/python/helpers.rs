...
pub fn resolve_py_type(rust_type: &str, all: &[TypeInfo], prefix: &str) -> String {
    ...
    if rt == "bool" || rt == "BOOLEAN" {
        return "bool".into();
    }
    if rt == "i8" || rt == "i16" || rt == "i32" || rt == "i64" {
        return "int".into();
    }
    if rt == "u8" || rt == "u16" || rt == "u32" || rt == "u64" {
        return "int".into();
    }
    if rt == "f32" || rt == "f64" {
        return "float".into();
    }
    if rt == "BIT_STRING" || rt.starts_with("BitString") || rt.starts_with("FixedBitString") {
        return "int".into();
    }
    if rt == "OctetString" || rt.starts_with("FixedOctetString") {
        return "bytes".into();
    }
    if rt.starts_with("VisibleString") || rt.starts_with("UTF8String") || rt == "String" {
        return "str".into();
    }
    if rt.starts_with("Vec <") || rt.starts_with("SequenceOf <") {
        return "list".into();
    }
    if rt.starts_with(prefix)
        || all
            .iter()
            .any(|t| format!("{}{}", prefix, t.name) == rt || t.name == rt)
    {
        if rt.starts_with(prefix) {
            return rt.to_string();
        }
...
fn list_elem_type(rust_type: &str, prefix: &str, all: &[TypeInfo]) -> Option<String> {
    ...
    let inner = if rt.starts_with("Vec <") {
        ...
        rt.trim_start_matches("SequenceOf <")
            .trim_end_matches('>')
            .trim()
    } else {
        return None;
    ...
    if all
        .iter()
        .any(|t| format!("{}{}", prefix, t.name) == with_prefix || t.name == inner)
    {
        Some(with_prefix)
    } else {
        None
    ...
    if f.is_list {
        ...
        if let Some(sz) = f.size_from_attr.filter(|&s| s > 0 && s <= 100) {
            if let Some(elem) = list_elem_type(&f.rust_type, prefix, all) {
                return format!(
                    "    {}: {} = field(default_factory=lambda: [{}() for _ in range({})])",
                    name, py_type, elem, sz
                );
            }
        }
            ...
            let fixed = f
                ...
                .or_else(|| {
                    ...
                    if f.rust_type.starts_with("FixedOctetString") {
                        let d: String =
                            f.rust_type.chars().filter(|c| c.is_ascii_digit()).collect();
                        d.parse::<usize>().ok()
                    } else {
                        None
                    }
