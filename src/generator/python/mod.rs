    ...
    for i in 0..n {
        let deps: Vec<String> = match &types[i].kind {
            ...
            TypeKind::Struct { fields } => fields
                .iter()
                .flat_map(|f| extract_dep(&f.rust_type, prefix, &all_names))
                .collect(),
            TypeKind::Choice { variants } => variants
                .iter()
                .flat_map(|v| extract_dep(&v.inner_type, prefix, &all_names))
                .collect(),
        };
        for dep in deps {
...
pub fn generate(
    ...
    // _native.py
    fs::write(
        src_dir.join("_native.py"),
        &native_gen::gen_native(&cfg.prefix, &cfg.package, &cfg.default_enc),
    )
    .expect("failed to write _native.py");

    // _base.py
    fs::write(
        src_dir.join("_base.py"),
        &native_gen::gen_base(&cfg.prefix, &cfg.package),
    )
    ...
    if !pixi_path.exists() {
        fs::write(
            &pixi_path,
            &native_gen::gen_pixi_toml(&cfg.prefix, &cfg.package),
        )
    ...
    all_types_code.push_str(
        &include_str!("templates/_types_header.py.txt")
    ...
    fs::write(src_dir.join("_types.py"), &all_types_code).expect("failed to write _types.py");
    ...
    init_code.push_str("]\n");
    fs::write(src_dir.join("__init__.py"), &init_code).expect("failed to write __init__.py");

    // Generate test file
    let mut all_tests = String::new();
    all_tests.push_str(
        &include_str!("templates/test_header.txt")
            .replace("__VER__", &super::java::helpers::gen_version()),
    );
    for ti in types {
    ...
    all_tests.push_str(")\n\n");
    for ti in types {
        all_tests.push_str(&test_gen::gen_test(
            ti,
            types,
            &cfg.prefix,
            &cfg.package,
    ...
    fs::write(test_dir.join("test_types.py"), &all_tests).expect("failed to write test_types.py");
