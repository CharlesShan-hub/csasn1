...
pub fn gen_base(_prefix: &str, _package: &str) -> String {
    ...
    c.push_str("import typing as _typing\n\n");
    c.push_str("def _resolve_type(tp):\n");
    c.push_str("    if isinstance(tp, _typing.ForwardRef):\n");
    c.push_str("        name = tp.__forward_arg__\n");
    c.push_str("        import sys as _sys\n");
    c.push_str("        for _m in ['cms_data._types', '__main__']:\n");
    c.push_str("            _mod = _sys.modules.get(_m)\n");
    c.push_str("            if _mod and hasattr(_mod, name):\n                return getattr(_mod, name)\n");
    c.push_str("    return tp\n\n");
    c.push_str("def _from_dict(data, cls) -> Any:\n");
    c.push_str("    if data is None:\n        return None\n");
    c.push_str("    cls = _resolve_type(cls)\n");
    ...
    c.push_str("            result = _from_dict(val, _resolve_type(fields['value'].type))\n");
    c.push_str("            result = _convert_bytes(result, _resolve_type(fields['value'].type))\n");
    c.push_str("            return cls(value=result)\n");
    c.push_str("        # CHOICE: data has a single key matching a variant\n");
    c.push_str("        if hasattr(cls, '_choice') and isinstance(data, dict):\n");
    c.push_str("            for key in data:\n");
    c.push_str("                if key in fields and key != '_choice':\n");
    c.push_str("                    val = _from_dict(data[key], _resolve_type(fields[key].type))\n");
    c.push_str("                    return cls(**{key: val, '_choice': key})\n");
    ...
    c.push_str("                    resolved = _resolve_type(field_type.type)\n");
    c.push_str("                    val = _from_dict(data[name], resolved)\n");
    c.push_str("                    kwargs[name] = _convert_bytes(val, resolved)\n");
