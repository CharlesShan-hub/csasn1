...
pub fn generate(_ti: &TypeInfo, all: &[TypeInfo], prefix: &str, cn: &str, variants: &[VariantInfo]) -> String {
    ...
    // testEncodeDecode (first variant) — skipped for choice types due to JER CHOICE complexity
    // Choice types require special JSON format that differs from Jackson serialization.
    c
}
