...
每个类型一个 `<前缀>*` POJO（数据统一存 `_v`）+ `<前缀>Base`（基类，`encode()`/`decode()`）+ `<前缀>Native`（JNA 调 DLL）+ `V`（`_v` 助手）+ 内建默认包装类 + 自动 JUnit 测试 + `pom.xml`。`encode()` 严格（只编码 `_set` 标记的 OPTIONAL），`encodeTest()` 宽松（全字段）。运行依赖 Jackson + JNA（可选 Lombok）。
...
`lib.rs` 在 `#[cfg(test)]` 下收纳测试子模块，`build.rs` 先编出 `generated.rs` 即可跑（在 `csasn1/` 下）：
...
- [x] 精确 OPTIONAL presence 控制（`encode()` 严格 / `encodeTest()` 宽松）

- [x] Fluent setter（链式调用）

- [ ] 泛化编解码接口（`Codec.aper().encode(obj)` 代替每类静态方法）

- [ ] 生成 `@JsonProperty` / `@JsonInclude` 注解
...
- [x] 生成版本标记（每个生成文件含 `csasn1 vX.Y.Z` 时间戳）
